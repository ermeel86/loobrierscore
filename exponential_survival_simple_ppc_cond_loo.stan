...
generated quantities {
    ...
    for(i in 1:N_uncensored) log_lik[i] = exponential_lpdf(times_uncensored[i] | exp(intercept+X_uncensored[i]*betas));
    for(i in 1:N_censored) log_lik[i+N_uncensored] = exponential_lccdf(times_censored[i] | exp(intercept + X_censored[i] * betas));
}
/**************************************************************************************/
