...
surv_df <- bind_rows(dplyr::select(surv_df, t, x0) %>% rename(surv=x0) %>% mutate(race="white"), 
                     dplyr::select(surv_df, t, x1)  %>% rename(surv=x1) %>% mutate(race="non-white"))
knitr::kable(
  head(surv_df,10)
)
...
iters <- sample(1:2000, 100)
log_ratios <- -post[iters,1:2,sprintf("log_lik[%d]",1:N)]
...
survs_loo1 <- surv_const_loo(times_pec,
                            X,
                            as.vector(post[iters,1:2,"intercept"]),
                            matrix(post[iters,1:2,sprintf("betas[%d]" ,1:ncol(X))], dim(post[iters, 1:2, ])[1]*dim(post[iters, 1:2, ])[2], ncol(X)),
                            t(w)
                            )
...
i_spline_basis_evals <- iSpline(times_pec, knots=knots, degree=mspline_degree,
                                intercept=FALSE, Boundary.knots = c(0, time_max))
log_ratios2 <- -post2[iters,1:2,sprintf("log_lik[%d]",1:N)]
...
survs_loo2 <- surv_mspline_loo(i_spline_basis_evals,
                            X,
                            matrix(post2[iters,1:2,sprintf("gammas[%d]", 1:nbasis)], dim(post2[iters,1:2,])[1]*dim(post2[iters,1:2,])[2], nbasis),
                            as.vector(post2[iters,1:2,"intercept"]),
                            matrix(post2[iters,1:2,sprintf("betas[%d]" ,1:ncol(X))], dim(post2[iters,1:2,])[1]*dim(post2[iters,1:2,])[2], ncol(X)),
                            t(w2)
                            )
...
pec_rslt <- pec(list("Constant-Loo"=survs_loo1, 
                     "Constant"=survs1
                     ), formula =Surv(time,status)~x , data=df, exact = F, times = times_pec,cens.model="marginal",
                splitMethod="none",
                B=0,
                verbose=TRUE)
