...
In this short case study I show how to implement a leave-one-out (LOO) version of the Expected Brier Score in General Survival Models with right-censored event times. We build on the so-called "inverse of probability of censoring weighted estimator" (below referred to as Brier Score) and use an efficient computation method to calculate LOO for all individuals using pareto-smoothed importance sampling. We provide an efficient implementation in R, based on exposed Stan code.
...
df <- as.tibble(bc)  %>%
  mutate(months = rectime*12/365)
```

```{r inspect, echo=FALSE}
knitr::kable(
  head(df,5), caption = 'Breast cancer survival data'
)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
N <- nrow(df)
X <- as.matrix(as.integer(pull(df, group))-1)
is_censored <- pull(df,censrec)==0
times <- pull(df,months)
...
surv_df <- as.tibble(t(survs)) %>%
  mutate(t=times_plot) %>%
  rename(x0=V1, x1=V2, x2=V3)

surv_df <- bind_rows(dplyr::select(surv_df, t, x0) %>% rename(surv=x0) %>% mutate(group="Good"), 
                     dplyr::select(surv_df, t, x1)  %>% rename(surv=x1) %>% mutate(group="Medium"),
                     dplyr::select(surv_df, t, x2)  %>% rename(surv=x2) %>% mutate(group="Poor")
                     )
knitr::kable(
  head(surv_df,10)
)
...
ggplot(data=surv_df) + 
  ...
  geom_line(mapping=aes(x=t, y=surv, color=group))+
...
pec_rslt <- pec(list("MSpline-Loo"=survs_loo2, 
                     ...
                     "MSpline"=survs2,
                     "Constant"=survs1
                     ), formula =Surv(months,censrec)~group , data=df, exact = F, times = times_pec,cens.model="marginal",
                splitMethod="none",
                B=0,
                verbose=TRUE)
